class Solution {
    func minimumPushes(_ word: String) -> Int {
        ...
        // var map = [Character: Int]() // Map Character to Key Pushes
        var totalPushes = 0
        var freqMap = word.reduce(into:[:]) { $0[$1, default: 0] += 1 }
        for (character,freq) in freqMap.sorted { $0.value > $1.value } {
            var key = heap.remove()!
            key.1 += 1
            heap.insert(key)
            totalPushes += (key.1 * freq)
        }
