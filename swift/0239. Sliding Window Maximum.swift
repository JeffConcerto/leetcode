class Solution {
    func maxSlidingWindow(_ nums: [Int], _ k: Int) -> [Int] {
        var heap: Heap<[Int]> = Heap { $0[0] > $1[0] }
        for i in 0..<k-1 {
        ...
        while left < nums.count {
          heap.insert([nums[left],left])
          var currentMax = heap.peek()!
          while currentMax[1] <= left-k {
              heap.remove()
              currentMax = heap.peek()!
          }
          result.append(currentMax[0])
          left += 1
        }
